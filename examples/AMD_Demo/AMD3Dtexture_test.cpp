...
namespace
{
	...
	class ExampleAMD3D : public entry::AppI
	{
	public:
		ExampleAMD3D(const char* _name, const char* _description, const char* _url)
		...
		void init(int32_t _argc, const char* const* _argv, uint32_t _width, uint32_t _height) override
		{
			...
			{
				const bgfx::Memory* memrgba8 = bgfx::alloc(32 * 32 * 32 * 4);
				const bgfx::Memory* memrgba16f = bgfx::alloc(32 * 32 * 32 * 2 * 4);
				...
				for (uint8_t zz = 0; zz < 32; ++zz)
				{
					for (uint8_t yy = 0; yy < 32; ++yy)
					{
						for (uint8_t xx = 0; xx < 32; ++xx)
						{
							...
							memrgba8->data[offset] = uint8_t(val << 3);
							*(uint16_t*)&memrgba16f->data[offset * 2] = bx::halfFromFloat((float)val / 32.0f);
							*(float*)&mem32f->data[offset * 4] = (float)val / 32.0f;
						}
					}
				}

				if (0 != (BGFX_CAPS_FORMAT_TEXTURE_3D & caps->formats[bgfx::TextureFormat::RGBA8]))
				{
					m_textures3d[m_numTextures3d++] = bgfx::createTexture3D(32, 32, 32, false, bgfx::TextureFormat::RGBA8, BGFX_SAMPLER_U_CLAMP | BGFX_SAMPLER_V_CLAMP | BGFX_SAMPLER_W_CLAMP, memrgba8);
				}

				if (0 != (BGFX_CAPS_FORMAT_TEXTURE_3D & caps->formats[bgfx::TextureFormat::RGBA16F]))
				{
					m_textures3d[m_numTextures3d++] = bgfx::createTexture3D(32, 32, 32, false, bgfx::TextureFormat::RGBA16F, BGFX_SAMPLER_U_CLAMP | BGFX_SAMPLER_V_CLAMP | BGFX_SAMPLER_W_CLAMP, memrgba16f);
				}
			...
			m_programCompute3D_RGBA8.idx = bgfx::kInvalidHandle;
			m_programCompute3D_RGBA16F.idx = bgfx::kInvalidHandle;
			m_programCompute3D_R32F.idx = bgfx::kInvalidHandle;
			if (m_computeSupported)
			{
				m_programCompute = bgfx::createProgram(loadShader("cs_update"), true);
				m_programCompute3D_RGBA8 = bgfx::createProgram(loadShader("cs_update3D_RGBA8"), true);
				m_programCompute3D_RGBA16F = bgfx::createProgram(loadShader("cs_update3D_RGBA16F"), true);
				m_programCompute3D_R32F = bgfx::createProgram(loadShader("cs_update3D_R32F"), true);
			}
		...
		virtual int shutdown() override
		{
			...
			if (bgfx::isValid(m_programCompute3D_RGBA8))
			{
				bgfx::destroy(m_programCompute3D_RGBA8);
			}
			if (bgfx::isValid(m_programCompute3D_RGBA16F))
			{
				bgfx::destroy(m_programCompute3D_RGBA16F);
			}

			if (bgfx::isValid(m_programCompute3D_R32F))
			{
				bgfx::destroy(m_programCompute3D_R32F);
			}
		...
		bool update() override
		{
			if (!entry::processEvents(m_width, m_height, m_debug, m_reset, &m_mouseState))
			{
				...
				if (bgfx::isValid(m_programCompute3D_RGBA8))
				{
					bgfx::setImage(0, m_textures3d[0], 0, bgfx::Access::Write, bgfx::TextureFormat::RGBA8);
					bgfx::dispatch(0, m_programCompute3D_RGBA8, 32, 32,32);
				}

				if (bgfx::isValid(m_programCompute3D_RGBA16F))
				{
					bgfx::setImage(0, m_textures3d[1], 0, bgfx::Access::Write, bgfx::TextureFormat::RGBA16F);
					bgfx::dispatch(0, m_programCompute3D_RGBA16F, 32, 32, 32);
				}

				if (bgfx::isValid(m_programCompute3D_R32F))
				{
					bgfx::setImage(0, m_textures3d[2], 0, bgfx::Access::Write, bgfx::TextureFormat::R32F);
					bgfx::dispatch(0, m_programCompute3D_R32F, 32, 32, 32);
				}
		...
		bgfx::ProgramHandle m_programCompute3D_RGBA8;
		bgfx::ProgramHandle m_programCompute3D_RGBA16F;
		bgfx::ProgramHandle m_programCompute3D_R32F;
