...
--			"Xbox360",
	...
	language "C++"
	startproject "example-AMD_Demo"

BGFX_DIR   = path.getabsolute("../3rd/bgfx")
...
function exampleProjectDefaults()
	...
	debugdir (path.join(MODULE_DIR, "examples/runtime"))

	includedirs {
		path.join(BIMG_DIR, "include"),
		path.join(BGFX_DIR, "include"),
		path.join(BGFX_DIR, "3rdparty"),
		path.join(MODULE_DIR, "examples/common"),
	}
	...
	configuration{"vs*"}
		targetdir (path.join(MODULE_DIR, "examples/runtime"))
		defines{
	...
	configuration { "xcode*", "ios" }
		kind "WindowedApp"
		files {
			path.join(MODULE_DIR, "examples/runtime/iOS-Info.plist"),
		}

	configuration { "xcode*", "tvos" }
		kind "WindowedApp"
		files {
			path.join(MODULE_DIR, "examples/runtime/tvOS-Info.plist"),
		}
...
function exampleProject(_combined, ...)
	...
	if _combined then
		...
		for _, name in ipairs({...}) do
			...
			files {
				path.join(MODULE_DIR, "examples", name, "**.c"),
				path.join(MODULE_DIR, "examples", name, "**.cpp"),
				path.join(MODULE_DIR, "examples", name, "**.h"),
				path.join(MODULE_DIR, "examples", name, "**.sc"),
				path.join(MODULE_DIR, "examples", name, "**.glsl"),
			}

			removefiles {
				path.join(MODULE_DIR, "examples", name, "**.bin.h"),
			}

		end

		files {
			path.join(MODULE_DIR, "examples/25-c99/helloworld.c"), -- hack for _main_
		}
